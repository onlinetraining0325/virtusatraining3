...
public class LeaveDetailsService {
    ...
    public String approveDeny(int leaveId,int mgrId, String status) {
        ...
        int leavail = employee.getLeaveAvail();
        int noOfdays = leaveDetails.getNoOfDays();
        if (mid != mgrId) {
            return "You are not Authorized to approve/deny the leave...";
        }
        if (status.equals("YES")) {
            ...
            leaveDetailsRepository.save(leaveDetails);
            leavail+=noOfdays;
            employee.setLeaveAvail(leavail);
    ...
    public List<LeaveDetails> showEmpPendingLeave(int empId) {
        List<LeaveDetails> leaves =  leaveDetailsRepository.findByEmpId(empId);
        return leaves.stream().filter(x -> x.getLeaveStatus().equals("PENDING")).collect(Collectors.toList());
    }
